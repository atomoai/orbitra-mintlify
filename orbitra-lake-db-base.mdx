...
```python
add_column_to_table(self, namespace: str, table_name: str, column: str, column_type: str) -> bool
```
...
```python
append_data(self, namespace: str, table_name: str, data: pd.DataFrame) -> str
```
...
- `data`: The DataFrame containing the data to append.

**Returns:**
- An operation ID for tracking the append operation.
...
```python
drop_table(self, namespace: str, table_name: str) -> bool
```

Drop a table from the given namespace.

**Args:**
- `namespace`: The namespace where the table resides.
...
**Raises:**
- `LakeError`: If the table or namespace does not exist.
...
Return the sqlglot dialect string for the given engine.

**Args:**
- `engine`: The engine to get the dialect for.

**Returns:**
- The sqlglot dialect string (e.g. "duckdb", "databricks").
...
which retrieves all columns. Restricting the projection to the needed columns can drastically
reduce the data read from wide tables.
...
```python
get_table_partition_hash_sums(self, namespace: str, table_name: str, partition_columns: list[str], partitions_filter: Optional[list[tuple]] = None) -> Optional[dict[tuple, Optional[int]]]
```
...
**Returns:**
- Optional\[dict\[tuple, Optional\[int]]]: A dict or None if the table is empty or the hash column doesn't exist.
...
```python
overwrite_data_by_custom_columns(self, namespace: str, table_name: str, custom_columns: list[str], data: pd.DataFrame, suppress_warnings: bool = False) -> str
```
...
- `data`: The DataFrame containing the data to overwrite.
- `suppress_warnings`: If True, silences the warning emitted when the custom columns do not
cover the table's partition columns and the overwrite scans the whole table. Defaults to False.
...
```python
set_column_descriptions(self, namespace: str, table_name: str, descriptions: dict[str, str]) -> bool
```

Set the descriptions of columns in a table.

Implementations should apply all descriptions in a single schema update, so that describing
N columns costs one metadata commit.

**Args:**
- `namespace`: The namespace where the table is located.
- `table_name`: The name of the table to set the column descriptions on.
- `descriptions`: A mapping of column name to description. An empty
string clears the description of that column.

**Returns:**
- True if the descriptions were set successfully.

**Raises:**
- `LakeError`: If the table or one of the columns does not exist.
...
```python
update_column_type(self, namespace: str, table_name: str, column: str, column_type: str) -> bool
```
...
- `column_type`: The new type of the column. A primitive type or a "decimal(P, S)" string.
...
#### `validate_overwrite_scope`

```python
validate_overwrite_scope(self, namespace: str, table_name: str) -> None
```

Refuse an overwrite whose replacement scope silently widened because the partition spec shrank.

The overwrite route and its granularity come from the table's current partition spec. When an external
engine removes a partition field (all of them, or one of a compound spec), the same call that used to
replace one partition replaces a whole former-partition group. The db-level overwrite methods run this
check before any write; callers with a short-circuit that may skip them (hash-based no-op) must run it first.

**Args:**
- `namespace`: The namespace where the table is located.
- `table_name`: The name of the table about to be overwritten.

**Raises:**
- `LakeError`: If the current partition source columns are a proper subset of an earlier spec's.
