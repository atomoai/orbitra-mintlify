...
Table and column descriptions are reconciled to match the declared schema: a description of
``None`` is left untouched, ``""`` clears it and any other value sets it.
...
```python
overwrite_data_by_custom_columns(self, namespace: str, table_name: str, custom_columns: list[str], df: pd.DataFrame, suppress_warnings: bool = False) -> OverwriteDataByCustomColumnsResponse
```
...
```python
update_column_type(self, namespace: str, table_name: str, column: str, column_type: str) -> TableSchema
```
